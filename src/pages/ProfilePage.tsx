...
import { jobStorage, applicationStorage } from '../utils/supabase-storage';
...
export function ProfilePage() {
  ...
  const validateForm = async (): Promise<boolean> => {
    ...
    if (!editData.contactNumber.trim()) {
      ...
      const users = await userProfileStorage.getAllUserProfiles();
      const cleanNumber = editData.contactNumber.replace(/\D/g, '');
      const existingUser = users.find((u: User) => u.contactNumber === cleanNumber && u.id !== user?.id);
      if (existingUser) {
