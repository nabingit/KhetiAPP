...
import { jobStorage, applicationStorage, userProfileStorage } from '../utils/supabase-storage';
import { SupabaseJobStatusManager } from '../utils/supabase-job-status-manager';
...
export function ApplicantsPage() {
  ...
  const loadData = async () => {
    ...
    await SupabaseJobStatusManager.updateAllJobStatuses();
    
    const allJobs = await jobStorage.getJobsWithAcceptedWorkers();
  ...
  const getWorkerProfile = async (workerId: string): Promise<User | null> => {
    return await userProfileStorage.getUserProfile(workerId);
  };

  const getWorkerJobsCompleted = async (workerId: string): Promise<number> => {
    const allApplications = await applicationStorage.getApplications();
    const allJobs = await jobStorage.getJobsWithAcceptedWorkers();
  ...
  const getWorkerTotalApplications = async (workerId: string): Promise<number> => {
    const allApplications = await applicationStorage.getApplications();
    return allApplications.filter(app => app.workerId === workerId).length;
  };

  const getWorkerSuccessRate = async (workerId: string): Promise<number> => {
    const totalApplications = await getWorkerTotalApplications(workerId);
    const completedJobs = await getWorkerJobsCompleted(workerId);
  ...
  const handleViewProfile = async (workerId: string) => {
    const workerProfile = await getWorkerProfile(workerId);
    if (workerProfile) {
  ...
  const handleUpdateApplication = async (applicationId: string, status: 'accepted' | 'rejected') => {
    ...
    if (status === 'accepted') {
      ...
      await applicationStorage.updateApplication(applicationId, { status: 'accepted' });
      ...
      await SupabaseJobStatusManager.checkStatusAfterWorkerAccepted(job.id);
      
    } else if (status === 'rejected') {
      // Update application with rejection timestamp
      await applicationStorage.updateApplication(applicationId, { 
        status: 'rejected',
  ...
  const handleMarkCompleted = async () => {
    ...
    await jobStorage.updateJob(job.id, { status: 'completed' });
    await loadData();
